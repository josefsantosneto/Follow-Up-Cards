...
mongoose.connect(
  "mongodb+srv://" +
    process.env.DB_KEY +
    "@cluster0.tnsju.mongodb.net/geosistemas",
        ...
        if (foundUser.type === "adm") {
          Project.find({}, (err, foundProject) => {
            ...
            Project.findOne(filter, (err, selectedProject) => {
              ...
              res.render("updateall", {
                projetos: foundProject,
...
app.get("/userperm", (req, res) => {
  if (req.isAuthenticated()) {
    ...
    User.findOne({ _id: userId }, (err, foundUser) => {
      if (foundUser.type === "adm") {
        const uTypes = [
          { selection: "Adminsitrador Geral", value: "adm" },
          { selection: "Visualizador de Projetos", value: "view" },
          {
...
app.get("/projetos", function (req, res) {
  if (req.isAuthenticated()) {
    ...
    Project.find(
      { $and: [filter, filter2, filter3] },
      function (err, foundProjects) {
        const selectList = [
          { selection: "Selecione um status", value: "all" },
          { selection: "Em Andamento", value: "emAndamento" },
          { selection: "Atrasado", value: "atrasado" },
          { selection: "Problema", value: "problema" },
          { selection: "Finalizado", value: "finalizado" },
          { selection: "Sem Status", value: "new" },
          { selection: "Todos", value: "all" },
        ];
...
app.get("/projetos/:projectId", function (req, res) {
  if (req.isAuthenticated) {
    ...
    Project.findOne({ _id: requestedProjectId }, function (err, project) {
      ...
      const valorTotalMedido = ar1.reduce((acc, cV) => {
        return acc + cV.valorMedicao;
      }, 0);
    ...
    User.findOne({ _id: userId }, (err, foundUser) => {
      if (err) {
        ...
        const project = new Project({
          ...
          centroCusto: centroCusto,
          orgao: orgao,
          valor: String(valor).toLocaleString("de-DE", {
